...
import pygame
import pickle  # for saving and loading nets
    ...
    def drawnet(self, screen, x, y, r):  # called from main to draw the selected net
        for i in range(len(self.outputs)):
            for j in range(len(self.outputs[i].inputs)):
                ...
                # print(f'L1: {l1}, N1: {n1}')
                n1 -= l1 * len(self.inputs)  # convert nodenum into position for net
                l2 = 2
                n2 = i
                # print(f'N1: {n1}, L1: {l1}')
                width = int(self.outputs[i].inputs[j][1] * 5)
                # print(self.outputs[i].inputs[j], self.outputs[i].inputs[j][1])
                ...
                #print(f'{colour}1')
                if colour == (0, 200, 200):
                    pygame.draw.line(screen, (0, 200, 0), (x + l1 * r * 8, y + n1 * r * 4),
                                     (x + l2 * r * 8, y + n2 * r * 4), width)  # l means 'layer', n means 'number'
                    #print('printblue')
                    #print((x + l1 * r * 8, y + n1 * r * 4), (x + l2 * r * 8, y + n2 * r * 4))
                else:
                    pygame.draw.line(screen, (200, 200, 0), (x + l1 * r * 8, y + n1 * r * 4),
                                     (x + l2 * r * 8, y + n2 * r * 4), width)  # l means 'layer', n means 'number'
                    #print('printred')
                    #print((x + l1 * r * 8, y + n1 * r * 4), (x + l2 * r * 8, y + n2 * r * 4))
                #print(f'{colour}2')
                # print(f'c {colour} l1 {l1} n1 {n1} l2 {l2} n2 {n2}')
        for i in range(len(self.hidden)):
...
# TODO: fix net draw colours

# TODO: maybe add inheritance cloning for speeeeed
# TODO: make it just generally less scuffed
# TODO: Something i discussed with tom i can't remember: I remember... file saving & loading and score adjustments
