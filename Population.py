...
class Population:
    ...
    def getfname(self, net, score=0, gen=0, i=0):
        fname = f'g{gen}s{score}i{len(net.inputs)}o{len(net.outputs)},i{i}'
        return fname

    def filesave(self, net, score=0, gen=0, i=0):
        fname = self.getfname(net, score, gen, i)
        try:
            f = open(f'{self.cwd}/SavedNets/{fname}', 'x')
        except FileExistsError:
            fname = f'{fname}+'
            f = open(f'{self.cwd}/SavedNets/{fname}', 'w')
            f.close()
        f = open(f'{self.cwd}/SavedNets/{fname}', 'wb')
        pickle.dump(net, f)
        f.close()

    def fileload(self, file):
        try:
            f = open(f'{self.cwd}/SavedNets/{file}', 'rb')
